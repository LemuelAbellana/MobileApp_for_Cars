...
export default function Inventory() {
  ...
  return <SafeAreaView edges={['left', 'right', 'bottom']} style={styles.screen}>
    <FlatList data={filtered} keyExtractor={car => String(car.id)} contentContainerStyle={[styles.content, { gap: 16, flexGrow: 1 }]}
      ...
      renderItem={({ item: car }) => <Pressable accessibilityRole="button" accessibilityLabel={`View ${car.year} ${car.brand} ${car.model}, ${formatPrice(car.price, displayCurrency, rates)}`} onPress={() => router.push(`/cars/${car.id}`)} style={({ pressed }) => [local.card, { opacity: pressed ? 0.8 : 1 }]}>
        <CarImage key={`${car.picture}:${loading}`} uri={car.picture} label={`${car.brand} ${car.model}`} />
        <View style={local.cardBody}><Text style={styles.subtitle}>{car.brand} {car.model}</Text><Text style={styles.price}>{formatPrice(car.price, displayCurrency, rates)}</Text><Text style={styles.muted}>{car.year} · {car.fuel_type} · {car.transmission}</Text></View>
      </Pressable>} />
  </SafeAreaView>;
}
